...
'''
vector form 
Y_i \ ~ \ N(X \beta, \ \sigma^2D(\theta)) \\
d_{ii} \ = exp[2log(PB_i)\theta]
...
# null is no change despite people making more
# hypothesis alternative is increase of about .5 per week for each increase in $1000 income 

glht(food.model, linfct = matrix(c(0,1), nrow = 1), rhs = 1, alternative = "less") %>% summary()
